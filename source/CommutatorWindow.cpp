...
void GuiWindows::ShowMultiplexerWindow(AppState& state) {
	...
	if (ImGui::BeginPopupModal("Connect?", NULL, ImGuiWindowFlags_AlwaysAutoResize))
	{
		...
		if (ImGui::Button("OK", ImVec2(120, 0))) {
			state.show_comm_progress_popup = true;
			state.comm_progress = 0.0f;
			state.comm_connection_success = false;
			state.comm_error_message.clear();
			state.comm_connection_future = std::async(std::launch::async, [&]() {
				try {
					state.comm_progress = 0.1f;
					if (comm_modbus.connect() == true) {
					...
					state.comm_progress = 0.3f;

					input_reg = comm_modbus.readInputRegisters(2, 1, 1);
					state.comm_progress = 0.6f;
					...
					state.comm_progress = 1.0f;
					state.comm_connection_success = true;
					std::this_thread::sleep_for(std::chrono::milliseconds(3000)); // Show completion briefly
				}
				catch (const std::exception& err) {
					state.comm_connection = false;
					state.comm_progress = 1.0f;
					state.comm_connection_success = false;
					state.comm_error_message = err.what();
				}
				});
	...
	// Progress popup
	if (state.show_comm_progress_popup) {
		...
		if (ImGui::BeginPopupModal("Connecting...", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
			if (state.comm_progress < 1.0f) {
				// Show progress bar while connecting
				ImGui::Text("Connecting to device...");
				ImGui::ProgressBar(state.comm_progress, ImVec2(200, 0));
			}
			else {
				// Operation complete - show result
				if (state.comm_connection_success) {
					ImGui::TextColored(ImVec4(0, 1, 0, 1), "Connection successful!");
					if (ImGui::Button("OK", ImVec2(120, 0))) {
						state.show_comm_progress_popup = false;
						ImGui::CloseCurrentPopup();
					}
				}
				else {
					ImGui::TextColored(ImVec4(1, 0, 0, 1), "Connection failed!");
					if (!state.comm_error_message.empty()) {
						ImGui::Text("Error: %s", state.comm_error_message.c_str());
					}
					if (ImGui::Button("OK", ImVec2(120, 0))) {
						state.show_comm_progress_popup = false;
						ImGui::CloseCurrentPopup();
			...
			// Check if the operation is complete (auto-close if successful)
			if (state.comm_progress >= 1.0f && state.comm_connection_success) {
				if (state.comm_connection_future.valid() &&
					state.comm_connection_future.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
					state.comm_connection_future.get();
				}
			}
